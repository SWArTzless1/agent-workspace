...
Open `projects/<project-name>/plans/<plan-name>.md` and read the following sections only:
...
**Always target the page explicitly when building — never rely on `penpot.root`.** `penpot.root` refers to whatever page happens to be open in the browser tab at that moment, not a page you can select programmatically — if the browser is still showing an old page while you build, everything lands there regardless of which page you intended to use. Before calling `execute_code` to create anything, fetch the specific target `Page` via `penpotUtils.getPageByName(<name>)` (or `getPageById`), then append every new shape to that page's own `.root` (`targetPage.root.appendChild(newBoard)`) — not to `penpot.root`. After building, verify placement with `penpotUtils.shapeStructure(targetPage.root, 2)` before considering the diagrams done — don't just trust that the code ran without error.
...
Once all items pass, write the solution to the `### Solution (Game Design Agent)` sub-section of the `## Game Design Plan` section in `projects/<project-name>/plans/<plan-name>.md`.
...
Written to `projects/<project-name>/docs/game-design.md` (or `projects/<project-name>/plans/<project-name>-gdd.md` if no project folder exists).
...
---
*Generated by Game Design Agent · Plan file: `projects/<project-name>/plans/<plan-name>.md`*
```
...
Write the artifact to `projects/<project-name>/plans/<project-name>-gdd.md` instead of `projects/<project-name>/docs/game-design.md`. Tell the user: "No project folder exists yet — GDD written to `projects/<project-name>/plans/<project-name>-gdd.md`. When a project folder is created, move this file to `projects/<project-name>/docs/game-design.md`."
